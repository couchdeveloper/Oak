    ...
    func testTSAN() async throws {
        ...
        enum T: EffectTransducer {
            ...
            enum State: Terminable, Equatable {
                case start, active(Int), finished
                var isTerminal: Bool { self == .finished }
            }
            enum Event { case start, payload(Payload) }
...
struct InternalContextTests {
        ...
        enum T: EffectTransducer {
            ...
            static func update(_ state: inout State, event: Event) -> T.Effect? {
                ...
                case .start:
                    ...
                    let operationEffect = T.Effect(
                        id: operationID,
                        isolatedOperation: { env, input, systemActor in
                ...
                case .spy:
                    ...
                    return T.Effect(f: { env, input, context, systemActor in
                        ...
                        #expect(
                            context.tasks.count == 1, "Context should contain exactly one task")

                        // 2. Expect that the task corresponds to our operation ID
                        #expect(
                            context.tasks[wrappedOperationID] != nil,
                            "Task should be registered with the specified ID")
    ...
    func verifyThatANewTaskWithSameIdWillCancelTheExistingTask() async throws {
        enum T: EffectTransducer {
            class Env {}
            enum State: Terminable {
                case start, spying, secondOperation, finished
                var isTerminal: Bool { self == .finished }
            }
            ...
            static func update(_ state: inout State, event: Event) -> T.Effect? {
                ...
                case .spy:
                    ...
                    return T.Effect(f: { env, input, context, systemActor in
                        ...
                        #expect(
                            context.tasks[wrappedOperationID] != nil,
                            "First task should be registered")
        ...
        enum T: EffectTransducer {
            ...
            static func update(_ state: inout State, event: Event) -> T.Effect? {
                ...
                case .spy:
                    ...
                    return T.Effect(f: { env, input, context, systemActor in
                        #expect(
                            context.tasks.count == 1,
                            "Context should contain exactly one task while operation is running"
                        )
                        #expect(
                            context.tasks[wrappedOperationID] != nil,
                            "Task should be registered while running")
                        return []
                    })
                ...
                case .operationComplete:
                    ...
                    return T.Effect(f: { env, input, context, systemActor in
                        ...
                        #expect(
                            ...
                            "Context should be empty after task completion")
                        #expect(
                            context.tasks[wrappedOperationID] == nil,
                            "Completed task should be removed from context")
    ...
    func verifyThatOnlyCorrectTaskIsRemovedDuringRaceCondition() async throws {
        enum T: EffectTransducer {
            class Env {}
            enum State: Terminable {
                case start, spying, secondTaskCreated, finished
                var isTerminal: Bool { self == .finished }
            }
            ...
            static func update(_ state: inout State, event: Event) -> T.Effect? {
                ...
                case .secondCreated:
                    ...
                    return T.Effect(f: { env, input, context, systemActor in
                        #expect(
                            context.tasks.count == 1,
                            "Should still have exactly one task (the second one)")
                        #expect(
                            context.tasks[wrappedOperationID] != nil,
                        ...
                        #expect(
                            context.tasks.count == 0,
                            "Context should be empty after second task completes")
