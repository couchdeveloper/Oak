...
struct TransducerViewTests {
    ...
    struct BasicInitializationTests {
        ...
        @Test func createVoidTransducer() async throws {
            ...
            let initialState: Binding<T.State> = .constant(.start)
            let observableTransducer = TransducerView(
                of: T.self,
            ...
            #expect(type(of: observableTransducer).Output.self == Void.self)
        }
        ...
        @Test func createOutputTransducer() async throws {
            ...
            #expect(type(of: observableTransducer).Output.self == Int.self)
        }
        ...
        @Test func createEffectTransducer() async throws {
            ...
            #expect(type(of: observableTransducer).Output.self == Void.self)
            #expect(type(of: observableTransducer).Env.self == Void.self)
        }
...
extension TransducerViewTests {
    ...
    struct Tests {
        ...
        func updateFunctionExecutesOnEventSendFromProxy() async throws {
            ...
            let view = TestView(initialState: TestTransducer.State.start) {
                TransducerView(
                    ...
                    Text(
                        verbatim:
                            "Test - Body executed \(tracker.increment()) times, state: \(state)")
                }
            }
        ...
        func stateUpdatesReflectInView() async throws {
            ...
            let view = TestView(initialState: CounterTransducer.State.count(0)) {
                TransducerView(
        ...
        func initialStateIsPreserved() async throws {
            ...
            let view = TestView(initialState: customInitialState) {
                TransducerView(
                    of: TestTransducer.self,
                    initialState: $0,
        ...
        func stateChangesTriggersViewUpdate() async throws {
            ...
            let view = TestView(initialState: ToggleTransducer.State.off) {
                TransducerView(
        ...
        func inputEventsReachTransducer() async throws {
            ...
            let view = TestView(initialState: EventTransducer.State.idle) {
                TransducerView(
        ...
        func multipleEventsProcessedInOrder() async throws {
            ...
            let view = TestView(initialState: SequenceTransducer.State.idle) {
                TransducerView(
        ...
        func outputsSentToSubject() async throws {
            ...
            let view = TestView(initialState: OutputTransducer.State.idle) {
                TransducerView(
        ...
        func initialOutputSent() async throws {
            ...
            let view = TestView(initialState: TestTransducer.State.idle) {
                TransducerView(
                    of: TestTransducer.self,
                    initialState: $0,
                    proxy: TestTransducer.Proxy(),
        ...
        func multipleOutputsDelivered() async throws {
            ...
            let view = TestView(initialState: MultiOutputTransducer.State.idle) {
                TransducerView(
        ...
        func completionCallbackInvokedOnSuccess() async throws {
            ...
            let view = TestView(initialState: CompletionTransducer.State.ready) {
                TransducerView(
        ...
        func transducerViewWorksWithNilProxy() async throws {
            ...
            let view = TestView(initialState: SimpleTransducer.State.idle) {
                TransducerView(
        ...
        func proxyChangeRestartsTransducerAndResetsState() async throws {
            ...
            let view = TestView(initialState: initialState) {
                TransducerView(
                    of: CounterTransducer.self,
                    initialState: $0,
                    proxy: currentProxy,
            ...
            hostingController.rootView = AnyView(
                TestView(initialState: initialState) {
                    TransducerView(
                        of: CounterTransducer.self,
                        initialState: $0,
                        proxy: currentProxy,
        ...
        func proxyChangeCancelsRunningTransducer() async throws {
            ...
            let view = TestView(initialState: LongRunningTransducer.State.running) {
                TransducerView(
            ...
            hostingController.rootView = AnyView(
                TestView(initialState: LongRunningTransducer.State.running) {
                    TransducerView(
                        of: LongRunningTransducer.self,
                        initialState: $0,
                        proxy: proxy2,
        ...
        func completionCallbackWithEffectTransducer() async throws {
            ...
            let view = TestView(initialState: EffectTestTransducer.State.ready) {
                TransducerView(
